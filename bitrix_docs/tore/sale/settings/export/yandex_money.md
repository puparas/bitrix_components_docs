# Автоматический экспорт каталогов в Яндекс.Маркет
...
Модуль торгового каталога имеет возможность выгрузки каталогов товаров в XML-файл, подходящий для размещения в системе  **Яндекс.Маркет**.

Укажите в [настройках](/user_help/store/catalog/settings_catalog.php) модуля **Торговый каталог** на закладке **Каталоги** те каталоги, которые необходимо выгружать в XML-файл. XML-файл будет пересоздаваться с периодичностью, указанной в переменной **Период экспорта в Яндекс.Маркет**. Готовый файл будет располагаться по адресу **http://*<ваш\_сайт>/<ваш\_каталог>*/yandex.php** (вместо подстроки
***<ваш\_сайт>*** должен быть указан реальный адрес сайта, подстрока ***<ваш\_каталог>*** берется из переменной **Путь по умолчанию для экспортируемых файлов**).

Готовый файл **http://*<ваш\_сайт>/<ваш\_путь>*/yandex.php** может принимать дополнительные параметры **referer1** и **referer2**, которые служат для формирования ссылок на товары с учётом рекламных компаний. Например, если запросить адрес:

```

http://<ваш_сайт>/<ваш_каталог>/yandex.php?referer1=wizard&referer2=soft
```

то в выдаваемом XML-файле ссылки на страницу с детальной информацией о товаре будут иметь вид:

```

http://<ваш_сайт>/catalog/detail.php?ID=8192&r1=wizard&r2=soft
```

Если параметр **referer1** не задан, то он принимает значение "yandext". Например,

```

http://<ваш_сайт>/catalog/detail.php?ID=8192&r1=yandext&r2=
```
