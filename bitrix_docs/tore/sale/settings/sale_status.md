...
| Действия | Действия над статусом:  * **Изменить параметры статуса** - [редактирование](/user_help/store/sale/settings/sale_status_edit.php) статуса; * **Удалить статус** - удаление статуса. |
| Код | Уникальный символьный код статуса. |
| Сорт. | Относительный "вес" статуса. Используется для определения положения данного статуса в общем списке. |
| Название | Название и описание статуса. Эти данные будут отображаться клиентам в публичном разделе сайта. |
| Тип статуса | Тип статуса заказа: **Заказ** или **Доставка**. |
| Цвет статуса | Цвет маркировки статуса, указанный при создании. |
| Уведомление | Если для статуса заказа настроен почтовый шаблон для отправки почтовых уведомлений покупателю, то отображается ведущая на этот шаблон ссылка **Да**. В противном случае отображается значение **Нет**. |

**Обратите внимание!** Существуют особые статусы заказов и отгрузок заказов, удаление которых невозможно:
...
* [Статусы заказов
  (учебный курс "Администратор. Бизнес")](https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=42&LESSON_ID=3077)
