...
Форма **Баллы за сообщения** (*Сервисы* > *Форумы* > *Баллы за сообщения*) предназначена для управления количеством баллов, присваиваемых пользователю форума за каждое отправленное сообщение. При помощи этой формы можно легко настроить кумулятивную систему начисления баллов (т.е. количество баллов за одно сообщение будет увеличиваться в зависимости от общего числа отправленных сообщений).
...
| Действия | Действия над записью о количестве баллов за сообщение:  * **Изменить параметры** - [редактирование](/user_help/service/forum/forum_points2post_edit.php) записи; * **Удалить** - удаление записи. |
| ID | Идентификатор записи в системе. |
| Необходимо сообщений | Число сообщений, начиная с которого за каждое сообщение будет начисляться указанное в записи количество баллов.     Например, если значение поля равно 10, то начиная с десятого сообщения посетителю будет начисляться новое (указанное в данной записи) количество баллов за каждое сообщение. |
