...
Форма **Перейти к инфоблоку / элементу** (Контент > Инфоблоки > Инструменты > Перейти к инфоблоку / элементу) позволяет, зная идентификатор инфоблока/элемента, быстро перейти к его форме редактирования.
...
| ID сущности | Задается идентификатор того вида сущности, которую необходимо найти. |

Если сущность с введенным идентификатором существует в системе и пользователь обладает необходимыми правами к этой сущности, то по кнопке **Найти** ему будет открыта страница искомой сущности (для инфоблока - страница со списком его разделов и элементов, для элемента - его форма редактирования). Если хотя бы одно из условий не будет выполнено, то будет выведено сообщение о том, что сущность не найдена.
