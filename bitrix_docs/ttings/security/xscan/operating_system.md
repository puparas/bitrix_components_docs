...
Операционная система (23.0.0)

# Операционная система

Помимо [поиска троянов](/user_help/settings/security/xscan/scanning_files.php) следует проверить наличие в операционной системе:
...
* посторонних
  ...
  SSH-ключи представляют собой пару – закрытый и открытый ключ. Закрытый должен храниться, соответственно, в закрытом доступе у клиента, а открытый отправляется на сервер и размещается в файле **authorized\_keys**.
  .
