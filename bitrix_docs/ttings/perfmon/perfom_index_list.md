...
Страница для анализа индексов. Список запросов доступен после сбора статистики по медленным запросам. Детальнее смотрите в [Курсе для хостеров](http://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=32&LESSON_ID=3798).
...
| Результат | Цветовой индикатор результата анализа:  * - индекс не создан * - индекс создан |
| Таблица | Имя таблицы в БД, к которой обращается запрос. |
| Колонки | Колонки таблицы в БД, к которым обращается запрос. |
...
| Общее время запроса | Общее время запроса. |
| Текст запроса | Собственно текст запроса. |
