...
На странице **SQL запросы** (*Настройки > Производительность > SQL запросы*) можно просмотреть отчет по запросам SQL.

  

**Примечание:** данная страница отображается, только если в настройках модуля **Монитор производительности** указана опция **Вести журнал запросов**.

**Примечание:** если в настройках модуля включена опция **Сохранять стек вызова для SQL запросов**, то при наведении указателя мыши виден стек запросов.

  

### Фильтр

Форма фильтра используется для фильтрации записей в соответствии с указанными условиями. Нижеследующая таблица описывает параметры, по которым могут выбираться записи.
...
| Хит | Позволяет найти запросы определенного хита. |
...
| # | Порядковый номер запроса того или иного хита. |
| Время | Время выполнения запроса. |
| Модуль | Модуль, которым был выполнен запрос. |
| Компонент | Компонент, который выполнял запрос. |
| Запрос | Код запроса. |
| База данных кластера | База данных, для которой был выполнен запрос (актуально только при установленном модуле **Веб-кластер**). |
