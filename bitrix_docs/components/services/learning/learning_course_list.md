...
### Описание **learning.course.list**

Компонент выводит список курсов. Компонент является стандартным и входит в дистрибутив модуля.
...
| Поле для cортировки | **SORTBY** | Указывается способ сортироки курсов в списке:  * **ID** – по идентификатору курса; * **NAME** – по названию; * **ACTIVE\_FORM** – по дате начала активности; * **SORT** – по индексу сортировки; * **TIMESTAMP\_X** – по дате последнего изменения. |
| Направление для cортировки | **SORTORDER** | Задается направление для cортировки:  * **ASC** – **По возрастанию**; * **DESC** – **По убыванию**. |
| Проверять право доступа | **CHECK\_PERMISSIONS** | [Y|N] При отмеченной опции будет проверяться право на доступ к курсу. |
| Количество курсов на странице | **COURSES\_PER\_PAGE** | Указывается количество курсов, выводимых на одной странице. Остальные курсы будут выведены с помощью постраничной навигации. |
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции в качестве заголовка страницы будет установлено **Список курсов**. |
