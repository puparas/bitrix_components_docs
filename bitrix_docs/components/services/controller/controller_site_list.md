...
### Описание **controller.site.list**

Компонент выводит список сайтов, зарегистрированных в контроллере. Компонент стандартный и входит в дистрибутив модуля.

**Примечание**: список сайтов будет отображен только для пользователей, имеющих уровень доступа **Авторизация на сайтах**.

В визуальном редакторе компонент расположен по пути *Сервисы > Контроллер > Список сайтов*.

Компонент относится к модулю [Контроллер](/user_help/service/controller/index.php).
...
| **Основные параметры** | | |
| Заголовок | **TITLE** | Указывается текст заголовка списка сайтов. |
| Группы сайтов для отображения | **GROUP** | Указываются группы, для которых будет отображен список сайтов, зарегистрированных в контроллере. |
