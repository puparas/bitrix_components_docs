...
[SOAP&WSDL сервер](/user_help/components/services/web_service/webservice_server.php)
[Веб-сервисы модуля статистики](/user_help/components/services/web_service/webservice_statistic.php)
[Проверка аутентификации](/user_help/components/services/web_service/webservice_checkauth.php)
...
### Описание **webservice.server**

Компонент реализует конечную точку (EndPoint) для сконфигурированного веб-сервиса на основе SOAP/WSDL. Компонент стандартный и входит в состав дистрибутива.

В визуальном редакторе компонент находится в *Компоненты > Сервисы > Веб-Сервис*.

Компонент относится к модулю [Веб-сервис](/user_help/settings/webservice/index.php).
...
| **Основные параметры** | | |
| Название веб-сервиса | **WEBSERVICE\_NAME** | Задаются имя используемого веб-сервиса. |
| Модуль веб-сервис | **WEBSERVICE\_MODULE** | Указывается модуль, который будет подключен при вызове сервиса. |
| Конфигурирующий класс | **WEBSERVICE\_CLASS** | Указывается собственно класс веб-сервиса. |
