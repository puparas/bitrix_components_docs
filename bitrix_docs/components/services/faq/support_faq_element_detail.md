...
### Описание **support.faq.element.detail**

Компонент выводит один вопрос FAQ из инфоблока конкретной категории. Компонент является стандартным и входит в дистрибутив модуля.
...
| ID Секции | **SECTION\_ID** | Указывается идентификатор секции (категории FAQ), в которой хранятся вопросы FAQ. |
| ID Элемента | **ELEMENT\_ID** | Указывается идентификатор элемента (вопроса FAQ). |
| **Настройки рейтингов** | | |
