...
### Описание **support.faq.section.list**

Компонент выводит список категории FAQ из инфоблока. Компонент является стандартным и входит в дистрибутив модуля.
...
| Список секций (по умолчанию - из корневой секции) | **SECTION** | Указывается секция (категория FAQ), которая будет отображена на странице. Если ничего не указывать, то будет выведен весь список секций. |
| Показывать вложенные секции | **EXPAND\_LIST** | [Y|N] При отмеченной секции будут показаны вложенные секции. |
| URL страницы секции (по умолчанию - из настроек инфоблока) | **SECTION\_URL** | Указывается адрес страницы секции FAQ. || **Управление режимом AJAX** | | |
