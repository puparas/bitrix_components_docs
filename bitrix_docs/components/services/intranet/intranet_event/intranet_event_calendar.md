...
### Описание **intranet.event\_calendar**
...
**Внимание!** Компонент устарел, рекомендуется использовать новый компонент [Календарь событий 2.0](/user_help/components/content/calendar/calendar.php).
...
| **Основные параметры** | | |
| Тип инфоблока | **IBLOCK\_TYPE** | Указывается тип информационного блока календаря событий. |
| Инфоблок | **IBLOCK\_ID** | Для выбранного типа инфоблоков указывается идентификатор инфоблока, в котором хранятся события. |
 Разрешить использовать избранные календари | **ALLOW\_SUPERPOSE** | [Y|N] При отмеченной опции доступен функционал избранных календарей. || Использовать возможность бронирования переговорных при создании событий | **ALLOW\_RES\_MEETING** | [Y|N] При отмеченной опции можно будет использовать возможность бронирования переговорных при создании событий. |
