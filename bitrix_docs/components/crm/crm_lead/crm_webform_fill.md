...
### Описание **crm.webform.fill**

Одностраничный компонент, отвечает за заполнение формы CRM-форм.

В визуальном редакторе компонент расположен по пути *CRM > Лиды > Заполнение формы CRM-форм*.
...
| **Основные параметры** | | |
| Код формы | **FORM\_ID** | Указывается уникальный идентификатор формы. |
| Символьный код формы | **FORM\_CODE** | Прописывается мнемонический идентификатор. Задается латинскими буквами, позволяет однозначно определить форму и вызвать ее из кода. |
| Код безопасности | **SECURITY\_CODE** | Вводится код для показа формы. Параметр берется из адреса страницы. |
| Путь к странице оплаты счета | **PATH\_TO\_INVOICE\_PAY** | Указывается путь к странице оплаты счета от корня сайта. Если страница находится в текущей директории, то достаточно указать ее название. |
