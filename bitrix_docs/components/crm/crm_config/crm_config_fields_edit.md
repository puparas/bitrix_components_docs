...
### Описание **crm.config.fields.edit**

Одностраничный компонент, позволяет выполнить настройку пользовательского поля в модуле CRM. Компонент стандартный и входит в дистрибутив модуля.

В визуальном редакторе компонент расположен по пути *CRM > Настройки > Настройка пользовательского поля*.
...
| Тип поля | **FIELS\_ENTITY\_ID** | Указывается тип дополнительного поля. Возможен выбор из трех вариантов:  * **Лид/контакт**; * **Компания**; * **Сделка**. |
| Дополнительное поле | **FIELS\_FIELD\_ID** | Имя переменной, где будет храниться мнемоническое имя поля. |
| **Шаблоны ссылок** | | |
| URL списка типов | **ENTITY\_LIST\_URL** | Указывается страница со списком типов полей. |
| URL списка полей | **FIELDS\_LIST\_URL** | Указывается страница со списком полей. |
| URL редактирования поля | **FIELD\_EDIT\_URL** | Указывается страница редактирования поля. |
