...
### Описание **crm.invoice.events**

Одностраничный компонент, выводит список действий, произведенных над счетами системы CRM. Компонент стандартный и входит в дистрибутив модуля.

В визуальном редакторе компонент расположен по пути *CRM > Просмотр событий счета*.
...
| **Основные параметры** | | |
| Тип сущности | **ENTITY\_TYPE** | Указывается тип сущностей, список действий над которыми будет выводить компонент. Доступно значение **Счет**. |
| ID элемента сущности | **ENTITY\_ID** | Указывается идентификатор счета. |
