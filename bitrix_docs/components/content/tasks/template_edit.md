...
| **Основные параметры** | | |
| Код | **TASK\_ID** | Указывается код, в котором передается идентификатор задачи. |
| Код пользователя | **USER\_ID** | Указывается код, в котором передается идентификатор пользователя. |
| Код группы | **GROUP\_ID** | Указывается код, в котором передается идентификатор группы социальной сети. |
| **Шаблоны ссылок** | | |
...
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции на странице будет установлен заголовок **Задача №<номер\_задачи>** или **Новая задача**. |
