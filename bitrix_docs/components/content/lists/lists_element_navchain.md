...
### Описание **lists.element.navchain**

Простой компонент добавляет к выводу обычного компонента **Навигационная цепочка (bitrix:breadcrumb)** элемент списка. Для работы компонента **bitrix:lists.element.navchain** в шаблон обязательно должен быть включен компонент **bitrix:breadcrumb**.

В визуальном редакторе компонент расположен по пути: *Контент > Универсальные списки > Навигационная цепочка*.
...
| URL разделы списка | **LIST\_URL** | Указывается шаблон ссылки на страницу разделов списка. |
...
**Примечание**: В данном компоненте невозможно использовать файлы **result\_modifier.php** и **component\_epilog.php**.
