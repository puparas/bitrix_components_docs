# Управление объединением профилей
...
### Описание **socserv.auth.split**

Компонент позволяет объединить профили пользователя (для социальных сетей). Компонент стандартный и входит в дистрибутив модуля.
...
| Показывать объединенные профили | **SHOW\_PROFILES** | [Y|N] При отмеченной опции профиль будет отображаться как единый для всех соцсетей. |
| Разрешить удалять объединенные профили | **ALLOW\_DELETE** | Разрешение на удаление объединённого профиля. |
