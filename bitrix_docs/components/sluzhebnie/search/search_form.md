...
### Описание **search.form**

В продукте форма поиска подключается в шаблоне дизайна и отображается на всех страницах портала. В строке ввода формы **Поиск по порталу** вводится поисковая фраза или слово и после нажатия кнопки **Поиск** пользователь перенаправляется на страницу **Поиск** (созданную с помощью компонента **Стандартная страница поиска (bitrix:search.page)**). Компонент стандартный и входит в дистрибутив модуля.
...
| **Шаблоны ссылок** | | |
| Страница выдачи результатов поиска (доступен макрос #SITE\_DIR#) | **PAGE** | Указывается путь к странице поиска (доступен макрос #SITE\_DIR#). |
