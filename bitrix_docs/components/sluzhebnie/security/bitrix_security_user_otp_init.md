...
### Описание **security.user.otp.init**

Компонент предназначен для вывода формы инициализации подключения нового устройства для двухэтапной авторизации.

В визуальном редакторе компонент находится в *Компоненты > Служебные > Управление резервными кодами > Подключение устройства OTP*.

Компонент относится к модулю [Проактивная защита](/user_help/settings/security/index.php).

  

**Примечание:** Тип алгоритма генерации паролей, который будет использоваться компонентом для подключения нового устройства задается на странице **настроек механизма двухэтапной авторизации** (*Настройки > Проактивная защита > Двухэтапная авторизация*).
...
| Путь для перенаправления после успешного подключения устройства | **SUCCESSFUL\_URL** | Указывается адрес, по которому будет перенаправлен пользователь после успешного подключения устройства. |
