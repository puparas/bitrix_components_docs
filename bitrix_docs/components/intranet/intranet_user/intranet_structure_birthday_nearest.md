...
### Описание **intranet.structure.birthday.nearest**

Компонент выводит список ближайших именинников офиса, отдела или филиала компании. Компонент является стандартным и входит в дистрибутив модуля.

В визуальном редакторе компонент находится в *Компоненты > Корпоративный портал > HR > Дни рождения*.
...
| Количество выводимых пользователей | **NUM\_USERS** | Параметр определяет количество отображаемых на странице записей. |
...
| Отображать фильтр | **SHOW\_FILTER** | [Y|N] При отмеченной опции будет отображатся панель выбора подразделения. Если опция не отмечена, то будет показан весь список именниников. |
| Показывать год рождения | **SHOW\_YEAR** | Поле определяет кому показываеть год рождения:  * **всем** (**Y**); * **только мужчинам** (**M**); * **никому** (**N**). |
| Пользовательские поля для вывода | **USER\_PROPERTY** | Указываются поля пользователей, которые будут показаны в визитных карточках сотрудников. |
| Отдел/офис | **DEPARTMENT** | Указывается офис, филиал или отдел, список ближайших именинников которого будет отображен. |
