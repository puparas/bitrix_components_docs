...
### Описание **sale.personal.subscribe.cancel**

Одностраничный компонент позволяет отменить подписку. Компонент стандартный и входит в дистрибутив модуля.

В визуальном редакторе компонент расположен по пути: *Магазин > Персональный раздел > Отмена подписки*.
...
| Страница со списком подписок | **PATH\_TO\_LIST** | Указывается путь к странице со списком подписок. Если страница находится в текущей директории, то достаточно указать ее название. Страница может быть создана с помощью компонента [Список подписок текущего пользователя](/user_help/store/sale/components_2/personal/sale_personal_subscribe_list.php). |
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции в качестве заголовка страницы будет установлено **Отмена подписки № <идентификатор\_подписки>**. |
| Идентификатор подписки | **ID** | Указывается код, результатом которого является получение идентификатора подписки. По умолчанию **={$ID}**. |
