...
### Описание **sale.personal.subscribe.list**

Одностраничный компонент выводит список подписок текущего пользователя. Компонент стандартный и входит в дистрибутив модуля.

В визуальном редакторе компонент расположен по пути: *Магазин > Персональный раздел > Список подписок текущего пользователя*.
...
| Страница отмены подписки | **PATH\_TO\_CANCEL** | Указывается путь к странице отмены подписки. Если страница находится в текущей директории, то достаточно указать ее название. Страница может быть создана с помощью компонента [Отмена подписки](/user_help/store/sale/components_2/personal/sale_personal_subscribe_cancel.php). Необходимо передавать идентификатор подписки. |
| Количество подписок, выводимых на странице | **PER\_PAGE** | Указывается количество подписок, отображаемых на одной странице. Все остальные подписки будут выведены с помощью постраничной навигации. |
