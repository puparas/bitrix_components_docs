...
### Описание **sale.personal.profile.list**

Одностраничный компонент выводит список профилей текущего пользователя. Компонент стандартный и входит в дистрибутив модуля.

В визуальном редакторе компонент расположен по пути: *Магазин > Персональный раздел > Список профилей текущего пользователя*.
...
| Страница с подробной информацией о профиле | **PATH\_TO\_DETAIL** | Указывается путь к странице редактирования или просмотра профиля. Если страница находится в текущей директории, то достаточно указать ее название. Страница может быть создана с помощью компонента [Редактирование профиля](/user_help/store/sale/components_2/personal/sale_personal_profile_detail.php). |
| Количество профилей, выводимых на странице | **PER\_PAGE** | Указывается количество профилей, отображаемых на одной странице. Все остальные профили будут выведены с помощью постраничной навигации. |
