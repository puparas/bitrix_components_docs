...
### Описание **sale.order.payment**

Одностраничный компонент осуществляет подключение платежной системы. Компонент применяется в тех случаях, когда обработчик платежной системы должен открываться в новом окне.

В визуальном редакторе компонент расположен по пути: *Магазин > Процедура оформления заказа > Подключение платежной системы*.
