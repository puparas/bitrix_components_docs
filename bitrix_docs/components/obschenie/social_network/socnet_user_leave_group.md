...
### Описание **socialnetwork.user\_leave\_group**

Компонент позволяет выйти из группы социальной сети. Компонент является стандартным и входит в дистрибутив модуля.

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент расположен по пути *Общение > Социальная сеть > Выход из группы*.
...
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции на странице в качестве заголовка будет установлено **<имя\_группы> : Выход из группы**. |
| **Имена переменных** | | |
