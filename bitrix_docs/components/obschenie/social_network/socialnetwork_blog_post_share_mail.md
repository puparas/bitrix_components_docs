# Уведомление о добавлении получателя
...
### Описание **socialnetwork.blog.post\_share.mail**

Компонент добавляет в почтовый шаблон уведомление о добавлении получателя.

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент расположен по пути *Общение > Социальная сеть > Уведомление о добавлении получателя*.
...
| ID сообщения | **POST\_ID** | Указывается идентификатор сообщения блога. |
| ID комментария | **COMMENT\_ID** | Указывается идентификатор комментария к сообщению блога. |
| URL сообщения | **URL** | Указывается URL сообщения блога в разделе сайта для почтовых пользователей (стандартно - в папке /pub/). |
