# Запросы на вступление в группу (ex)
...
### Описание **socialnetwork.group\_requests.ex**

Позволяет управлять приглашениями и запросами на вступление в группу. Компонент является стандартным и входит в дистрибутив модуля.

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент находится в *Общение > Социальная сеть > Запросы на вступление в группу*.
...
| ID группы соцсети | **GROUP\_ID** | Указывается идентификатор группы соцсети. |
| Путь к группе | **PATH\_TO\_GROUP** | Задается путь к странице группы социальной сети. |
| Шаблон пути к странице пользователя | **PATH\_TO\_USER** | Задается путь к странице профиля пользователя социальной сети. |
| Устанавливать цепочку навигации | **SET\_NAV\_CHAIN** | [Y|N] При отмеченной опции на странице будет установлена цепочка навигации. |
