...
### Описание **socialnetwork.log.ex**

Компонент получает список записей социальной сети для показа, а в его шаблоне подключаются компоненты [bitrix:socialnetwork.log.entry](/user_help/components/obschenie/social_network/socialnetwork_log_entry.php) и [bitrix:socialnetwork.blog.post](/user_help/components/obschenie/social_network/socialnetwork_blog_post.php), получающие список комментариев к каждой из записей и отображающие запись вместе с комментариями. При этом реализовано кеширование на уровне каждой записи Живой ленты, вместо компонентного AJAX используется API, реализованное непосредственно в компоненте.
  
Компонент содержит шаблон: **.default**. Компонент является стандартным и входит в дистрибутив модуля.
...
| **Источник данных** | | |
| Количество дней лога | **LOG\_DATE\_DAYS** | Количество дней ведения лога изменений социальной сети. |
| **Шаблоны ссылок** | | |
...
| Устанавливать цепочку навигации | **SET\_NAVCHAIN** | [Y|N] При отмеченной опции в цепочку навигации будет добавлен пункт **Лог обновлений**. |
| Учитывать подписку | **SUBSCRIBE\_ONLY** | [Y|N] При отмеченной опции будет отображаться лог изменений с учетом подписки, иначе - весь лог. |
| Количество отображаемых записей | **LOG\_CNT** | Указывается количество записей в логе изменений. |
| **Имена переменных** | | |
