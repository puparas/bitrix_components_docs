# Уведомление о записи или комментарии
...
### Описание **socialnetwork.log.entry.mail**

Компонент добавляет в почтовый шаблон уведомление о записи или комментарии.

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент расположен по пути *Общение > Социальная сеть > Уведомление о записи или комментарии*.
...
| Email получателя | **EMAIL\_TO** | Указывается email получателя сообщения. |
| ID получателя | **RECIPIENT\_ID** | Указывается идентификатор пользователя-получателя сообщения (почтового пользователя). |
| ID записи ЖЛ | **LOG\_ENTRY\_ID** | Указывается идентификатор записи Живой ленты. |
| ID комментария | **COMMENT\_ID** | Указывается идентификатор комментария к сообщению Живой ленты. |
| URL сообщения | **URL** | Указывается URL сообщения Живой ленты в разделе сайта для почтовых пользователей (стандартно - в папке /pub/). |
