...
### Описание **mobile.socialnetwork.log.ex**

Компонент отображает Живую ленту в формате для мобильных устройств. Компонент является стандартным и входит в дистрибутив модуля.
...
| **Источник данных** | | |
| Количество дней лога | **LOG\_DATE\_DAYS** | Указывается количество дней ведения лога изменений социальной сети. |
| **Внешний вид** | | |
| Отображать комментарии с привязкой к событиям | **USE\_COMMENTS** | [Y|N] При отмеченной опции комментарии будут отображаться с привязкой к событиям. |
| **Шаблоны ссылок** | | |
...
| Устанавливать цепочку навигации | **SET\_NAV\_CHAIN** | [Y|N] При отмеченной опции в цепочку навигации будет добавлен пункт **Лог обновлений**. |
...
| Имя переменной для идентификатора сообщения | **GROUP\_VAR** | Указывается имя переменной, которой передается идентификатор группы социальной сети. |
