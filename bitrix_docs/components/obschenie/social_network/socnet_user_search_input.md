...
### Описание **socialnetwork.user\_search\_input**

Компонент предназначен для вывода формы, с помощью которой осуществляется ввод пользователей социальной сети. Компонент является стандартным и входит в дистрибутив модуля.

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент расположен по пути *Общение > Социальная сеть > Форма ввода пользователей*.
...
| **Дополнительно** | | |
| Количество записей в выпадающем списке | **PAGE\_ELEMENTS** | Указывается количество отображаемых записей в выпадающем списке-подсказке уже существующих пользователей. |
| Дополнительные свойства | **TEXT** | Указывается код дополнительного свойства. |
| Вставлять фрейм в шаблон компонента | **TMPL\_IFRAME** | [Y|N] При отмеченной опции фрейм будет вставлен в шаблон компонента. |
| **Основные параметры** | | |
| Имя поля ввода | **NAME** | Задается название формы ввода пользователей. |
| Содержимое поля ввода | **VALUE** | Указывается содержимое формы ввода пользователей по умолчанию. |
| JavaScript функция | **FUNCTION** | Указывается имя функции, которой передается результат выбора пользователя. |
| Код экстранета (I/E/EA) | **EXTRANET** | Указывается код экстранета:  * **I** - сотрудники компании; * **EA** - все внешние пользователи; * **E** - доступные внешние пользователи. |
