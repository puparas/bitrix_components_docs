...
### Описание **socialnetwork.messages\_input**

Компонент позволяет показать входящие сообщения социальной сети. Компонент является стандартным и входит в дистрибутив модуля.

**Примечание:** компонент устарел в связи с выходом модуля Веб-мессенджер (с версии 11.5).

Компонент относится к модулю [Социальная сеть](/user_help/service/socialnetwork/index.php).

В визуальном редакторе компонент расположен по пути *Общение > Социальная сеть > Входящие сообщения*.
...
| **Внешний вид** | | |
| Количество записей в списке | **ITEMS\_COUNT** | Указывается количество входящих сообщений, отображаемых на одной странице. Все сообщения будут выведены с помощью постраничной навигации. |
| **Шаблоны ссылок** | | |
...
| Шаблон пути к странице ответа на сообщение | **PATH\_TO\_MESSAGE\_FORM\_MESS** | Задается путь к странице ответа на сообщение социальной сети. |
| Шаблон пути к входящим сообщениям | **PATH\_TO\_MESSAGES\_INPUT** | Задается путь к странице входящих сообщений социальной сети. |
| Шаблон пути к входящим сообщениям с фильтром по пользователю | **PATH\_TO\_MESSAGES\_INPUT\_USER** | Задается путь к входящим сообщениям с фильтром по пользователю социальной сети. |
| **Дополнительные настройки** | | |
| Устанавливать цепочку навигации | **SET\_NAV\_CHAIN** | [Y|N] При отмеченной опции в цепочку навигации будет добавлен пункт **Входящие сообщения**. |
...
| **Имена переменных** | | |
| Имя переменной для сообщения | **MESSAGE\_VAR** | Указывается имя переменной, которой передается идентификатор сообщения социальной сети. |
