...
### Описание **forum.topic.move**

Компонент служит для создания страницы перемещения тем форума. Компонент стандартный и входит в дистрибутив модуля.
...
| Страница переноса тем | **URL\_TEMPLATES\_TOPIC\_MOVE** | Указывается адрес страницы переноса темы форума. По умолчанию поле содержит **topic\_move.php?FID=#FID#&TID=#TID#.** |
...
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции в качестве заголовка страницы будет установлено **Перенос темы в другой форум**. |
