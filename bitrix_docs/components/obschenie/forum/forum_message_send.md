...
### Описание **forum.message.send**

Компонент выводит форму отправки письма (сообщения) пользователю. Компонент стандартный и входит в дистрибутив модуля.
...
| Могут отправлять сообщение (icq) из профиля | **SEND\_ICQ** | Указываются пользователи, которые могут отправлять сообщение (icq) из профиля. |
| **Шаблоны ссылок** | | |
...
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции в качестве заголовка страницы будет установлено **Отправка письма**. |
