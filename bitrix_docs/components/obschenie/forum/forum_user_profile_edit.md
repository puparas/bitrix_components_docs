...
### Описание **forum.user.profile.edit**

Компонент выводит форму редактирования профиля пользователя. Компонент стандартный и входит в дистрибутив модуля.
...
| **Основные параметры** | | |
| ID пользователя | **UID** | Указывается идентификатор пользователя, профиль которого будет изменяться. По умолчанию поле содержит **={$\_REQUEST["UID"]**. |
| **Шаблоны ссылок** | | |
...
| Показывать доп. свойства | **USER\_PROPERTY** | Указываются дополнительные пользовательские свойства, которые будут отображены в публичной части в секции **Дополнительно**. |
...
| Устанавливать заголовок страницы | **SET\_TITLE** | [Y|N] При отмеченной опции в качестве заголовка страницы будет установлено **<имя\_пользователя> (изменение профиля)**. |
