...
Компонент представляет собой сборник js-библиотек служебных шаблонов, которые часто используются в компонентах.
...
Например, с помощью одной из таких библиотек прорисовывается выпадающее меню со списком форумов:

```
<?$APPLICATION->IncludeComponent("bitrix:forum.interface","popup",
